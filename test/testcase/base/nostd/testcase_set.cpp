...
void test_set() {
    ...
    auto lambda_test = [&](const char* text, t_set<int>& set) -> void {
        t_range_set<uint32> part1;
        part1.add(7, 12).add(14).add(15, 16).add(17, 18).add(21).add(19).add(22).add(20).add(25).add(23).add(24);
        t_range_set<uint32> part2;
        part2.add(7, 12).add(14).add(16).add(17, 18).add(19, 22).add(23, 24).add(26, 30);
        part1.subtract(part2).for_each([](uint32 begin, uint32 end) -> void {
            if (begin == end) {
                _logger->writeln("> %u", begin);
        ...
        expect.add(15).add(25);
        _test_case.assert(part1 == expect, __FUNCTION__, text);
    };

    lambda_test("range set", range_set);
    lambda_test("bit set", bit_set);
}
...
void dump(t_range_set<t_range_value<T>>& rs, basic_stream& dbs) {
    ...
    rs.for_each2([&](t_interval<t_range_value<T>> interval) -> void {
        dbs.printf("%s", (interval.begin_flag == range_flag_t::included) ? "[" : "(");
        ...
        dbs.printf("%s", (interval.end_flag == range_flag_t::included) ? "]" : ")");
    });
}
...
void test_range_set4() {
    _test_case.begin("set");
    {
        ...
        expect  //
            .clear()
            .add(interval(1.0, 1.5, range_flag_t::included, range_flag_t::excluded))
            .add(interval(3.5, 4.0, range_flag_t::excluded, range_flag_t::included));
