...
void test_rfc7516_A1() {
    ...
    jose.encrypt(context, jwe_t::jwe_a256gcm, jwa_t::jwa_rsa_oaep, tobin(input), compact, jose_serialization_t::jose_compact);
    ...
    jose.encrypt(context, jwe_t::jwe_a256gcm, jwa_t::jwa_rsa_oaep, tobin(input), json_flat, jose_serialization_t::jose_flatjson);
    ...
    jose.encrypt(context, jwe_t::jwe_a256gcm, jwa_t::jwa_rsa_oaep, tobin(input), json_serial, jose_serialization_t::jose_json);
...
void test_rsa_oaep() {
    ...
    jose.open(&context, &key);
    jose.encrypt(context, jwe_t::jwe_a128gcm, jwa_t::jwa_rsa_oaep, tobin(input), output, jose_serialization_t::jose_compact);
...
void test_rsa_oaep_256() {
    ...
    jose.open(&context, &key);
    jose.encrypt(context, jwe_t::jwe_a256gcm, jwa_t::jwa_rsa_oaep_256, tobin(input), output, jose_serialization_t::jose_compact);
    ret = jose.decrypt(context, output, plain, result);
    jose.close(context);
...
void test_rfc7516_A2() {
    ...
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_rsa_1_5, tobin(input), compact, jose_serialization_t::jose_compact);
    ret = jose.decrypt(context, compact, source, result);
    _test_case.test(ret, __FUNCTION__, "RFC 7516 A.2. JWE using RSAES-PKCS1-v1_5 and AES_128_CBC_HMAC_SHA_256 (compact)");

    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_rsa_1_5, tobin(input), json_flat, jose_serialization_t::jose_flatjson);
    ret = jose.decrypt(context, compact, source, result);
    _test_case.test(ret, __FUNCTION__, "RFC 7516 A.2. JWE using RSAES-PKCS1-v1_5 and AES_128_CBC_HMAC_SHA_256 (flat)");

    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_rsa_1_5, tobin(input), json_serial, jose_serialization_t::jose_json);
    ret = jose.decrypt(context, compact, source, result);
...
void test_rfc7516_A3() {
    ...
    jose_context_t* context = nullptr;
    jose.open(&context, &key);
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_a128kw, tobin(input), compact, jose_serialization_t::jose_compact);
    ret = jose.decrypt(context, compact, source, result);
    _test_case.test(ret, __FUNCTION__, "RFC 7516 A.3. JWE using AES Key Wrap and AES_128_CBC_HMAC_SHA_256 (compact)");
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_a128kw, tobin(input), json_flat, jose_serialization_t::jose_flatjson);
    ret = jose.decrypt(context, json_flat, source, result);
    _test_case.test(ret, __FUNCTION__, "RFC 7516 A.3. JWE using AES Key Wrap and AES_128_CBC_HMAC_SHA_256 (flat)");
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_a128kw, tobin(input), json_serial, jose_serialization_t::jose_json);
    ret = jose.decrypt(context, json_serial, source, result);
...
void test_rfc7516_A4() {
    ...
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, algs, tobin(input), json_serial, jose_serialization_t::jose_json);
    ret = jose.decrypt(context, json_serial, source, result);
...
void test_rfc7517_C() {
    ...
    jose.open(&context, &key);
    jose.encrypt(context, jwe_t::jwe_a128cbc_hs256, jwa_t::jwa_pbes2_hs256_a128kw, tobin(input), output, jose_serialization_t::jose_compact);
    ret = jose.decrypt(context, output, plain, result);
    jose.close(context);
        ...
        for (size_t i = 0; i < RTL_NUMBER_OF(encs); i++) {
            ...
            for (size_t j = 0; j < RTL_NUMBER_OF(algs); j++) {
                ...
                if (nameof_alg) {
                    ...
                    ret = jose.encrypt(handle_encrypt, enc, alg, tobin(input), encrypted, jose_serialization_t::jose_flatjson);
                    if (errorcode_t::success == ret) {
...
void test_jwe_json(jwe_t enc) {
    ...
    __try2 {
        ...
        ret = jose.encrypt(handle_encrypt, enc, algs, tobin(input), encrypted, jose_serialization_t::jose_json);
        if (errorcode_t::success != ret) {
            __leave2;
        }
...
void test_rfc8037() {
    ...
    ret = jose.encrypt(handle, jwe_t::jwe_a128gcm, jwa_t::jwa_ecdh_es_a128kw, tobin(claim), encrypted, jose_serialization_t::jose_flatjson);
    if (errorcode_t::success == ret) {
    ...
    ret = jose.encrypt(handle, jwe_t::jwe_a256gcm, jwa_t::jwa_ecdh_es_a256kw, tobin(claim), encrypted, jose_serialization_t::jose_flatjson);
    if (errorcode_t::success == ret) {
...
void test_okp() {
    ...
    for (size_t i = 0; i < RTL_NUMBER_OF(encs); i++) {
        ...
        for (size_t j = 0; j < RTL_NUMBER_OF(algs); j++) {
            ...
            ret = jose.encrypt(handle, encs[i], algs[j], tobin(claim), encrypted, jose_serialization_t::jose_flatjson);
            if (errorcode_t::success == ret) {
                dump("encrypted", encrypted);
