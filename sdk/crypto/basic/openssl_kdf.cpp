...
return_t openssl_kdf::hkdf_label(binary_t& hkdflabel, uint16 length, const char* label, const binary_t& context) {
    return_t ret = errorcode_t::success;
    __try2 {
        hkdflabel.clear();
        ...
        ret = hkdf_label(hkdflabel, length, str2bin(label), context);
    }
    __finally2 {
        // do nothing
    }
...
return_t openssl_kdf::hkdf_label(binary_t& hkdflabel, uint16 length, const binary_t& label, const binary_t& context) {
    return_t ret = errorcode_t::success;
    __try2 {
        hkdflabel.clear();
        ...
        binary_append(hkdflabel, "tls13 ", 6);
...
return_t openssl_kdf::hkdf_expand_label(binary_t& okm, const char* alg, uint16 length, const binary_t& secret, const char* label, const binary_t& context) {
    return_t ret = errorcode_t::success;
    __try2 {
        okm.clear();

        if (nullptr == alg || nullptr == label) {
            ret = errorcode_t::invalid_parameter;
            __leave2;
        }
        if (context.size() > 255) {
        ...
        ret = hkdf_label(bin_hkdflabel, length, label, context);
...
return_t openssl_kdf::hkdf_expand_label(binary_t& okm, hash_algorithm_t alg, uint16 length, const binary_t& secret, const char* label,
