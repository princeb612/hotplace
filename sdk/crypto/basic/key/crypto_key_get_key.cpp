    ...
    if (errorcode_t::success == ret) {
        switch (type) {
            ...
            case crypto_kty_t::kty_slhdsa: {
                // pub
                iter = datamap.find(crypt_item_t::item_pubkey);
                if (datamap.end() != iter) {
                    pub1 = iter->second;
                }
            } break;
...
return_t crypto_key::get_private_key(const EVP_PKEY* pkey, binary_t& priv) {
    ...
    if (errorcode_t::success == ret) {
        switch (type) {
            ...
            case crypto_kty_t::kty_mldsa:
            case crypto_kty_t::kty_slhdsa: {
                iter = datamap.find(crypt_item_t::item_privkey);
                if (datamap.end() != iter) {
                    priv = iter->second;
                }
            } break;
...
return_t crypto_key::get_key(const EVP_PKEY* pkey, int flags, crypto_kty_t& type, binary_t& pub1, binary_t& pub2, binary_t& priv, bool preserve) {
    ...
    if (errorcode_t::success == ret) {
        ...
        switch (type) {
            ...
            case crypto_kty_t::kty_slhdsa: {
                if (public_key & flags) {
                    lambda_get_item(crypt_item_t::item_pubkey, pub1);
                }
                if (private_key & flags) {
                    lambda_get_item(crypt_item_t::item_privkey, priv);
                }
            } break;
            default:
                break;
