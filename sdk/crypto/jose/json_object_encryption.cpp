...
return_t json_object_encryption::encrypt (jose_context_t* handle, jwe_t enc, std::list <jwa_t> algs, binary_t const& input, std::string& output, jose_serialization_t type)
{
    return_t ret = errorcode_t::success;
    json_object_encryption encryption;
...
return_t json_object_encryption::encrypt (jose_context_t* handle, jwe_t enc, jwa_t alg, binary_t const& input, binary_t& output)
{
    ...
    __try2
    {
        ...
        // alg part - encrypted_key from cek
        {
            ...
            if (crypto_key_t::kty_hmac == alg_info->kty) {
            ...
            if (jwa_type_t::jwa_type_rsa == alg_type) {
                ...
                if (1 == handle->encryptions.size ()) {
                    /* compact, flattened */
                    std::string header;
                    update_header (item.header, tag1, aad, header);
                    if (header.size ()) {
                        item.header = header;
                    }
                }
...
return_t json_object_encryption::update_header (std::string const& source_encoded, binary_t const& tag, binary_t& aad, std::string& output_encoded)
{
    return_t ret = errorcode_t::success;
...
return_t json_object_encryption::prepare_encryption (jose_context_t* handle, jwe_t enc, std::list <jwa_t> const& algs)
{
    return_t ret = errorcode_t::success;
    openssl_prng rand;
    crypto_advisor* advisor = crypto_advisor::get_instance ();
...
return_t json_object_encryption::prepare_encryption_recipient (jwa_t alg, EVP_PKEY* pkey, jose_recipient_t& recipient, crypt_datamap_t& datamap, crypt_variantmap_t& variantmap)
{
    return_t ret = errorcode_t::success;
    crypto_advisor* advisor = crypto_advisor::get_instance ();
...
return_t json_object_encryption::compose_encryption_header (binary_t& header, jwe_t enc, jwa_t alg, jose_compose_t flag, std::string const& kid, uint32 flags)
{
    return_t ret = errorcode_t::success;
    crypt_datamap_t datamap;
    crypt_variantmap_t variantmap;
...
return_t json_object_encryption::compose_encryption_header (binary_t& header, jwe_t enc, jwa_t alg, jose_compose_t flag, std::string const& kid, crypt_datamap_t& datamap, crypt_variantmap_t& variantmap, uint32 flags)
{
    return_t ret = errorcode_t::success;
    json_t* json_header = nullptr;
    crypto_advisor* advisor = crypto_advisor::get_instance ();
...
return_t json_object_encryption::prepare_decryption (jose_context_t* handle,
...
return_t json_object_encryption::prepare_decryption_recipient (jose_context_t* handle,
...
return_t json_object_encryption::prepare_decryption (jose_context_t* handle, const char* input)
{
    return_t ret = errorcode_t::success;
...
return_t json_object_encryption::write_encryption (jose_context_t* handle, std::string& output, jose_serialization_t type)
{
    return_t ret = errorcode_t::success;
    crypto_advisor* advisor = crypto_advisor::get_instance ();
