...
class cbor_web_key : public crypto_keychain {
   public:
     ...
     * @param const crypto_key* cryptokey [in]
     * @param keyflag_t mode [in] see keyflag_t
     * @param stream_t* stream [in]
     * @param int flags [in] public_key | private_key
     * @return error code (see error.hpp)
     */
    virtual return_t write(const crypto_key* cryptokey, keyflag_t mode, stream_t* stream, int flags = public_key);
    /**
     * @brief   write
     * @param   const crypto_key* crypto_key [in]
     * @param   stream_t* stream [in]
     * @param   int flags [inopt] public_key | private_key
     * @return  error code (see error.hpp)
     */
    return_t write(const crypto_key* cryptokey, stream_t* stream, int flags = public_key);
    /**
     * @brief   write
     * @param   const crypto_key* crypto_key [in]
     ...
     * @return  error code (see error.hpp)
     */
    return_t write(const crypto_key* crypto_key, std::string& buf, int flags = public_key);
    /**
     * @brief   write
     * @param   const crypto_key* crypto_key [in]
     ...
     * @return  error code (see error.hpp)
     */
    return_t write(const crypto_key* crypto_key, binary_t& cbor, int flags = public_key);
    /**
     ...
     *          root->release ();
     */
    return_t write(const crypto_key* crypto_key, cbor_object** root, int flags = public_key);
    /**
     * @brief   diagnostic
     * @param   const crypto_key* crypto_key [in]
     ...
     * @param   int flags [inopt] public_key | private_key
     */
    return_t diagnose(const crypto_key* crypto_key, stream_t* stream, int flags = public_key);
