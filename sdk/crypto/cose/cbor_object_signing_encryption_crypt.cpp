...
/**
 * @file   cbor_object_signing_encryption_crypt.cpp
...
return_t cbor_object_signing_encryption::docrypt(cose_context_t* handle, crypto_key* key, cose_layer* layer, cose_mode_t mode) {
    ...
    __try2 {
        ...
        if (istraceable(trace_category_t::trace_category_crypto)) {
            trace_debug_event(trace_category_t::trace_category_crypto, trace_event_t::trace_event_cose_encryption,
                              [&](basic_stream& dbs) -> void { dbs.println("docrypt alg %i (%s)", alg, hint->name); });
        }
#endif
        ...
        if (mode) {
            body.get_params().finditem(cose_param_plaintext, input);
        } else {
            body.get_payload().get(payload);
        }
        ...
        if (cose_group_t::enc_aesgcm == group) {
            ...
            encrypt_option_t options[] = {
                ...
                {crypt_ctrl_t::nsize, hint->enc.nsize},
                {},
            };
            if (mode) {
        ...
        if (mode) {
            ...
            layer->setparam(cose_param_ciphertext, output);
        }
    }
    __finally2 {}
