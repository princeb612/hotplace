/* vim: set tabstop=4 shiftwidth=4 softtabstop=4 expandtab smarttab : */
/**
 * @file   resource_cose.cpp
 * @author Soo Han, Kim (princeb612.kr@gmail.com)
 * @desc https://www.iana.org/assignments/cose/cose.xhtml
 *
 * Revision History
 * Date         Name                Description
 */

#include <hotplace/sdk/base/nostd/enumclass.hpp>
#include <hotplace/sdk/crypto/advisor/crypto_advisor.hpp>
#include <hotplace/sdk/io/system/sdk.hpp>
#include <iostream>

namespace hotplace {
namespace crypto {

/* keep order by group id */
const hint_cose_group_t hint_cose_groups[] = {
    {
        cose_group_t::sign_ecdsa,  // 1
        crypt_category_t::sign,
        cose_hint_sign | cose_hint_kty_ec,
    },
    {
        cose_group_t::sign_eddsa,  // 2
        crypt_category_t::sign,
        cose_hint_sign | cose_hint_kty_okp,
    },
    {
        cose_group_t::mac_hmac,  // 3
        crypt_category_t::mac,
        cose_hint_mac,
    },
    {
        cose_group_t::mac_aes,  // 4
        crypt_category_t::mac,
        cose_hint_mac,
    },
    {
        cose_group_t::enc_aesgcm,  // 5
        crypt_category_t::crypt,
        cose_hint_enc | cose_hint_iv,
    },
    {
        cose_group_t::enc_aesccm,  // 6
        crypt_category_t::crypt,
        cose_hint_enc | cose_hint_iv,
    },
    {
        cose_group_t::enc_chacha20_poly1305,  // 7
        crypt_category_t::crypt,
        cose_hint_enc | cose_hint_not_supported | cose_hint_iv,
    },
    {
        cose_group_t::key_direct,  // 8
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_kty_oct,
    },
    {
        cose_group_t::key_hkdf_hmac,  // 9
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_kty_oct,
    },
    {
        cose_group_t::key_hkdf_aes,  // 10
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_kty_oct,
    },
    {
        cose_group_t::key_aeskw,  // 11
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_kek | cose_hint_kty_oct,
    },
    {
        cose_group_t::key_ecdhes_hmac,  // 12
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_epk | cose_hint_kty_ec,
    },
    {
        cose_group_t::key_ecdhss_hmac,  // 13
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_static_key | cose_hint_static_kid | cose_hint_kty_ec,
    },
    {
        cose_group_t::key_ecdhes_aeskw,  // 14
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_kek | cose_hint_epk | cose_hint_kty_ec,
    },
    {
        cose_group_t::key_ecdhss_aeskw,  // 15
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_salt | cose_hint_party | cose_hint_kek | cose_hint_static_key | cose_hint_static_kid | cose_hint_kty_ec,
    },
    {
        cose_group_t::sign_rsassa_pss,  // 16
        crypt_category_t::sign,
        cose_hint_sign | cose_hint_kty_rsa,
    },
    {
        cose_group_t::key_rsa_oaep,  // 17
        crypt_category_t::keydistribution,
        cose_hint_agree | cose_hint_kek | cose_hint_kty_rsa,
    },
    {
        cose_group_t::sign_rsassa_pkcs15,  // 18
        crypt_category_t::sign,
        cose_hint_sign | cose_hint_kty_rsa,
    },
    {
        cose_group_t::iv_generate,  // 19
        crypt_category_t::unknown,
    },
    {
        cose_group_t::hash,  // 20
        crypt_category_t::hash,
    },
    {
        cose_group_t::sign_mldsa,
        crypt_category_t::sign,
        cose_hint_sign | cose_hint_kty_mldsa,
    },
};

const size_t sizeof_hint_cose_groups = RTL_NUMBER_OF(hint_cose_groups);

constexpr const hint_cose_group_t* link_cose_group(cose_group_t g) { return hint_cose_groups + (t_underlying(g) - 1); }

const hint_cose_algorithm_t hint_cose_algorithms[] = {
    {
        cose_alg_t::cose_aes128kw,
        "A128KW",
        crypto_kty_t::kty_oct,
        cose_group_t::key_aeskw,
        link_cose_group(cose_group_t::key_aeskw),
        {},
        {},
        {
            "aes-128-wrap",
            128 >> 3,  // 16
        },
    },
    {
        cose_alg_t::cose_aes192kw,
        "A192KW",
        crypto_kty_t::kty_oct,
        cose_group_t::key_aeskw,
        link_cose_group(cose_group_t::key_aeskw),
        {},
        {},
        {
            "aes-192-wrap",
            192 >> 3,  // 24
        },
    },
    {
        cose_alg_t::cose_aes256kw,
        "A256KW",
        crypto_kty_t::kty_oct,
        cose_group_t::key_aeskw,
        link_cose_group(cose_group_t::key_aeskw),
        {},
        {},
        {
            "aes-256-wrap",
            256 >> 3,  // 32
        },
    },
    {
        cose_alg_t::cose_direct,
        "direct",
        crypto_kty_t::kty_oct,
        cose_group_t::key_direct,
        link_cose_group(cose_group_t::key_direct),
    },
    {
        cose_alg_t::cose_es256,
        "ES256",
        crypto_kty_t::kty_ec,
        cose_group_t::sign_ecdsa,
        link_cose_group(cose_group_t::sign_ecdsa),
        {
            NID_X9_62_prime256v1,
            cose_ec_curve_t::cose_ec_p256,
        },
    },
    {
        cose_alg_t::cose_es384,
        "ES384",
        crypto_kty_t::kty_ec,
        cose_group_t::sign_ecdsa,
        link_cose_group(cose_group_t::sign_ecdsa),
        {
            NID_secp384r1,
            cose_ec_curve_t::cose_ec_p384,
        },
    },
    {
        cose_alg_t::cose_es512,
        "ES512",
        crypto_kty_t::kty_ec,
        cose_group_t::sign_ecdsa,
        link_cose_group(cose_group_t::sign_ecdsa),
        {
            NID_secp521r1,
            cose_ec_curve_t::cose_ec_p521,
        },
    },
    {
        cose_alg_t::cose_eddsa,
        "EdDSA",
        crypto_kty_t::kty_eddsa,
        cose_group_t::sign_eddsa,
        link_cose_group(cose_group_t::sign_eddsa),
        {
            NID_ED25519,
            cose_ec_curve_t::cose_ec_ed25519,
        },
    },
    {
        cose_alg_t::cose_hkdf_sha256,
        "direct+HKDF-SHA-256",
        crypto_kty_t::kty_oct,
        cose_group_t::key_hkdf_hmac,
        link_cose_group(cose_group_t::key_hkdf_hmac),
        {},
        {
            "sha256",
            256 >> 3,  // 32
        },
        {
            nullptr,
            128 >> 3,
        },
    },
    {
        cose_alg_t::cose_hkdf_sha512,
        "direct+HKDF-SHA-512",
        crypto_kty_t::kty_oct,
        cose_group_t::key_hkdf_hmac,
        link_cose_group(cose_group_t::key_hkdf_hmac),
        {},
        {
            "sha512",
            512 >> 3,  // 64
        },
        {
            nullptr,
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_hkdf_aes128,
        "direct+HKDF-AES-128",
        crypto_kty_t::kty_oct,
        cose_group_t::key_hkdf_aes,
        link_cose_group(cose_group_t::key_hkdf_aes),
        {},
        {
            "aes-128-cbc",
            32,
        },
        {
            nullptr,
            128 >> 3,
        },
    },
    {
        cose_alg_t::cose_hkdf_aes256,
        "direct+HKDF-AES-256",
        crypto_kty_t::kty_oct,
        cose_group_t::key_hkdf_aes,
        link_cose_group(cose_group_t::key_hkdf_aes),
        {},
        {
            "aes-256-cbc",
            64,
        },
        {
            nullptr,
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha1,
        "SHA-1",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha1",
            160 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha256_64,
        "SHA-256/64",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha256",
            84 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha256,
        "SHA-256",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha256",
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha512_256,
        "SHA-512/256",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha512",
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha384,
        "SHA-384",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha384",
            384 >> 3,
        },
    },
    {
        cose_alg_t::cose_sha512,
        "SHA-512",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "sha512",
            512 >> 3,
        },
    },
    {
        cose_alg_t::cose_shake128,
        "SHAKE128",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "shake128",
        },
    },
    {
        cose_alg_t::cose_shake256,
        "SHAKE256",
        crypto_kty_t::kty_oct,
        cose_group_t::hash,
        link_cose_group(cose_group_t::hash),
        {},
        {
            "shake256",
        },
    },
    {
        cose_alg_t::cose_ecdhes_hkdf_256,
        "ECDH-ES + HKDF-256",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhes_hmac,
        link_cose_group(cose_group_t::key_ecdhes_hmac),
        {
            NID_X9_62_prime256v1,
            cose_ec_curve_t::cose_ec_p256,
        },
        {
            "sha256",
            256 >> 3,  // 32
        },
        {
            nullptr,
            128 >> 3,
        },
    },
    {
        cose_alg_t::cose_ecdhes_hkdf_512,
        "ECDH-ES + HKDF-512",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhes_hmac,
        link_cose_group(cose_group_t::key_ecdhes_hmac),
        {
            NID_secp521r1,
            cose_ec_curve_t::cose_ec_p521,
        },
        {
            "sha512",
            512 >> 3,  // 64
        },
        {
            nullptr,
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_ecdhss_hkdf_256,
        "ECDH-SS + HKDF-256",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhss_hmac,
        link_cose_group(cose_group_t::key_ecdhss_hmac),
        {
            NID_X9_62_prime256v1,
            cose_ec_curve_t::cose_ec_p256,
        },
        {
            "sha256",
            256 >> 3,  // 32
        },
        {
            nullptr,
            128 >> 3,
        },
    },
    {
        cose_alg_t::cose_ecdhss_hkdf_512,
        "ECDH-SS + HKDF-512",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhss_hmac,
        link_cose_group(cose_group_t::key_ecdhss_hmac),
        {
            NID_secp521r1,
            cose_ec_curve_t::cose_ec_p521,
        },
        {
            "sha512",
            512 >> 3,  // 64
        },
        {
            nullptr,
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_ecdhes_a128kw,
        "ECDH-ES + A128KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhes_aeskw,
        link_cose_group(cose_group_t::key_ecdhes_aeskw),
        {
            NID_X9_62_prime256v1,
            cose_ec_curve_t::cose_ec_p256,
        },
        {
            "sha256",
            128 >> 3,  // 16
        },
        {
            "aes-128-wrap",  // -3
        },
    },
    {
        cose_alg_t::cose_ecdhes_a192kw,
        "ECDH-ES + A192KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhes_aeskw,
        link_cose_group(cose_group_t::key_ecdhes_aeskw),
        {
            NID_secp384r1,
            cose_ec_curve_t::cose_ec_p384,
        },
        {
            "sha256",
            192 >> 3,  // 24
        },
        {
            "aes-192-wrap",  // -4
        },
    },
    {
        cose_alg_t::cose_ecdhes_a256kw,
        "ECDH-ES + A256KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhes_aeskw,
        link_cose_group(cose_group_t::key_ecdhes_aeskw),
        {
            NID_secp521r1,
            cose_ec_curve_t::cose_ec_p521,
        },
        {
            "sha256",
            256 >> 3,  // 32
        },
        {
            "aes-256-wrap",  // -5
        },
    },
    {
        cose_alg_t::cose_ecdhss_a128kw,
        "ECDH-SS + A128KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhss_aeskw,
        link_cose_group(cose_group_t::key_ecdhss_aeskw),
        {
            NID_X9_62_prime256v1,
            cose_ec_curve_t::cose_ec_p256,
        },
        {
            "sha256",
            128 >> 3,  // 16
        },
        {
            "aes-128-wrap",  // -3
        },
    },
    {
        cose_alg_t::cose_ecdhss_a192kw,
        "ECDH-SS + A192KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhss_aeskw,
        link_cose_group(cose_group_t::key_ecdhss_aeskw),
        {
            NID_secp384r1,
            cose_ec_curve_t::cose_ec_p384,
        },
        {
            "sha256",
            192 >> 3,  // 24
        },
        {
            "aes-192-wrap",  // -4
        },
    },
    {
        cose_alg_t::cose_ecdhss_a256kw,
        "ECDH-SS + A256KW",
        crypto_kty_t::kty_ec,
        cose_group_t::key_ecdhss_aeskw,
        link_cose_group(cose_group_t::key_ecdhss_aeskw),
        {
            NID_secp521r1,
            cose_ec_curve_t::cose_ec_p521,
        },
        {
            "sha256",
            256 >> 3,  // 32
        },
        {
            "aes-256-wrap",  // -5
        },
    },
    {
        cose_alg_t::cose_ps256,
        "RSA-PSS-256",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pss,
        link_cose_group(cose_group_t::sign_rsassa_pss),
    },
    {
        cose_alg_t::cose_ps384,
        "RSA-PSS-384",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pss,
        link_cose_group(cose_group_t::sign_rsassa_pss),
    },
    {
        cose_alg_t::cose_ps512,
        "RSA-PSS-512",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pss,
        link_cose_group(cose_group_t::sign_rsassa_pss),
    },
    {
        cose_alg_t::cose_rsaoaep1,
        "RSA-OAEP",
        crypto_kty_t::kty_rsa,
        cose_group_t::key_rsa_oaep,
        link_cose_group(cose_group_t::key_rsa_oaep),
    },
    {
        cose_alg_t::cose_rsaoaep256,
        "RSA-OAEP-256",
        crypto_kty_t::kty_rsa,
        cose_group_t::key_rsa_oaep,
        link_cose_group(cose_group_t::key_rsa_oaep),
    },
    {
        cose_alg_t::cose_rsaoaep512,
        "RSA-OAEP-512",
        crypto_kty_t::kty_rsa,
        cose_group_t::key_rsa_oaep,
        link_cose_group(cose_group_t::key_rsa_oaep),
    },
    {
        cose_alg_t::cose_es256k,
        "ES256K",
        crypto_kty_t::kty_ec,
        cose_group_t::sign_ecdsa,
        link_cose_group(cose_group_t::sign_ecdsa),
        {
            NID_secp256k1,
            cose_ec_curve_t::cose_ec_secp256k1,
        },
    },
#if OPENSSL_VERSION_NUMBER >= 0x30500000L
    {
        cose_alg_t::cose_mldsa44,
        "ML-DSA-44",
        crypto_kty_t::kty_mldsa,
        cose_group_t::sign_mldsa,
        link_cose_group(cose_group_t::sign_mldsa),
        {NID_ML_DSA_44},
    },
    {
        cose_alg_t::cose_mldsa65,
        "ML-DSA-65",
        crypto_kty_t::kty_mldsa,
        cose_group_t::sign_mldsa,
        link_cose_group(cose_group_t::sign_mldsa),
        {NID_ML_DSA_65},
    },
    {
        cose_alg_t::cose_mldsa87,
        "ML-DSA-87",
        crypto_kty_t::kty_mldsa,
        cose_group_t::sign_mldsa,
        link_cose_group(cose_group_t::sign_mldsa),
        {NID_ML_DSA_87},
    },
#endif
    {
        cose_alg_t::cose_rs256,
        "RS256",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pkcs15,
        link_cose_group(cose_group_t::sign_rsassa_pkcs15),
    },
    {
        cose_alg_t::cose_rs384,
        "RS384",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pkcs15,
        link_cose_group(cose_group_t::sign_rsassa_pkcs15),
    },
    {
        cose_alg_t::cose_rs512,
        "RS512",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pkcs15,
        link_cose_group(cose_group_t::sign_rsassa_pkcs15),
    },
    {
        cose_alg_t::cose_rs1,
        "RS1",
        crypto_kty_t::kty_rsa,
        cose_group_t::sign_rsassa_pkcs15,
        link_cose_group(cose_group_t::sign_rsassa_pkcs15),
    },
    {
        cose_alg_t::cose_aes128gcm,
        "A128GCM",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesgcm,
        link_cose_group(cose_group_t::enc_aesgcm),
        {},
        {},
        {
            "aes-128-gcm",
            128 >> 3,  // 16
            16,
        },
    },
    {
        cose_alg_t::cose_aes192gcm,
        "A192GCM",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesgcm,
        link_cose_group(cose_group_t::enc_aesgcm),
        {},
        {},
        {
            "aes-192-gcm",
            192 >> 3,  // 24
            16,
        },
    },
    {
        cose_alg_t::cose_aes256gcm,
        "A256GCM",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesgcm,
        link_cose_group(cose_group_t::enc_aesgcm),
        {},
        {},
        {
            "aes-256-gcm",
            256 >> 3,  // 32
            16,
        },
    },
    {
        cose_alg_t::cose_hs256_64,
        "HS256/64",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_hmac,
        link_cose_group(cose_group_t::mac_hmac),
        {},
        {
            "sha256",
            64 >> 3,  // 8
            256 >> 3,
        },
        {
            nullptr,
            128 >> 3,
        },
    },
    {
        cose_alg_t::cose_hs256,
        "HS256/256",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_hmac,
        link_cose_group(cose_group_t::mac_hmac),
        {},
        {
            "sha256",
            256 >> 3,  // 32
            256 >> 3,
        },
        {
            nullptr,
            256 >> 3,
        },
    },
    {
        cose_alg_t::cose_hs384,
        "HS384/384",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_hmac,
        link_cose_group(cose_group_t::mac_hmac),
        {},
        {
            "sha384",
            384 >> 3,  // 48
            384 >> 3,
        },
        {
            nullptr,
            384 >> 3,
        },
    },
    {
        cose_alg_t::cose_hs512,
        "HS512/512",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_hmac,
        link_cose_group(cose_group_t::mac_hmac),
        {},
        {
            "sha512",
            512 >> 3,  // 64
            512 >> 3,
        },
        {
            nullptr,
            512 >> 3,
        },
    },
    {
        cose_alg_t::cose_aesccm_16_64_128,
        "AES-CCM-16-64-128",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-128-ccm",
            128 >> 3,        // 16
            64 >> 3,         // 8
            15 - (16 >> 3),  // len(IV) = 15-2 = 13
        },
    },
    {
        cose_alg_t::cose_aesccm_16_64_256,
        "AES-CCM-16-64-256",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-256-ccm",
            256 >> 3,        // 32
            64 >> 3,         // 8
            15 - (16 >> 3),  // len(IV) = 15-2 = 13
        },
    },
    {
        cose_alg_t::cose_aesccm_64_64_128,
        "AES-CCM-64-64-128",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-128-ccm",
            128 >> 3,        // 16
            64 >> 3,         // 8
            15 - (64 >> 3),  // len(IV) = 15-8 = 7
        },
    },
    {
        cose_alg_t::cose_aesccm_64_64_256,
        "AES-CCM-64-64-256",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-256-ccm",
            256 >> 3,        // 32
            64 >> 3,         // 8
            15 - (64 >> 3),  // len(IV) = 15-8 = 7
        },
    },
    {
        cose_alg_t::cose_aesccm_16_128_128,
        "AES-CCM-16-128-128",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-128-ccm",
            128 >> 3,        // 16
            128 >> 3,        // 16
            15 - (16 >> 3),  // len(IV) = 15-2 = 13
        },
    },
    {
        cose_alg_t::cose_aesccm_16_128_256,
        "AES-CCM-16-128-256",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-256-ccm",
            256 >> 3,        // 32
            128 >> 3,        // 16
            15 - (16 >> 3),  // len(IV) = 15-2 = 13
        },
    },
    {
        cose_alg_t::cose_aesccm_64_128_128,
        "AES-CCM-64-128-128",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-128-ccm",
            128 >> 3,        // 16
            128 >> 3,        // 16
            15 - (64 >> 3),  // len(IV) = 15-8 = 7
        },
    },
    {
        cose_alg_t::cose_aesccm_64_128_256,
        "AES-CCM-64-128-256",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_aesccm,
        link_cose_group(cose_group_t::enc_aesccm),
        {},
        {},
        {
            "aes-256-ccm",
            256 >> 3,        // 32
            128 >> 3,        // 16
            15 - (64 >> 3),  // len(IV) = 15-8 = 7
        },
    },
    {
        cose_alg_t::cose_aesmac_128_64,
        "AES-MAC-128/64",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_aes,
        link_cose_group(cose_group_t::mac_aes),
        {},
        {},
        {
            "aes-128-cbc",
            128 >> 3,  // 16
            64 >> 3,   // 8
        },
    },
    {
        cose_alg_t::cose_aesmac_256_64,
        "AES-MAC-256/64",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_aes,
        link_cose_group(cose_group_t::mac_aes),
        {},
        {},
        {
            "aes-256-cbc",
            256 >> 3,  // 32
            64 >> 3,   // 8
        },
    },
    {
        cose_alg_t::cose_aesmac_128_128,
        "AES-MAC-128/128",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_aes,
        link_cose_group(cose_group_t::mac_aes),
        {},
        {},
        {
            "aes-128-cbc",
            128 >> 3,  // 16
            128 >> 3,  // 16
        },
    },
    {
        cose_alg_t::cose_aesmac_256_128,
        "AES-MAC-256/128",
        crypto_kty_t::kty_oct,
        cose_group_t::mac_aes,
        link_cose_group(cose_group_t::mac_aes),
        {},
        {},
        {
            "aes-256-cbc",
            256 >> 3,  // 32
            128 >> 3,  // 16
        },
    },
    {
        cose_alg_t::cose_chacha20_poly1305,
        "ChaCha20/Poly1305",
        crypto_kty_t::kty_oct,
        cose_group_t::enc_chacha20_poly1305,
        link_cose_group(cose_group_t::enc_chacha20_poly1305),
        {},
        {},
        {
            "chacha20-poly1305",
            32,
            16,
        },
    },
#if 0
    {
        cose_alg_t::cose_iv_generation,
        "IV-GENERATION",
        crypto_kty_t::kty_oct,
        cose_group_t::iv_generate,
        link_cose_group(cose_group_t::iv_generate),
    },
#endif
};

const size_t sizeof_hint_cose_algorithms = RTL_NUMBER_OF(hint_cose_algorithms);

}  // namespace crypto
}  // namespace hotplace
