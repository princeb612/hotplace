...
return_t authenticode_plugin_pe::write_authenticode (file_stream* filestream, binary_t data)
{
    ...
    __try2
    {
        ...
        if ((WIN_CERT_REVISION_2_0 != win_certificate->wRevision) ||
            (WIN_CERT_TYPE_PKCS_SIGNED_DATA != win_certificate->wCertificateType)) {
            ret = errorcode_t::bad_format;
            __leave2;
        }
...
bool authenticode_plugin_pe::separated ()
{
    return false;
}
...
authenticode_engine_id_t authenticode_plugin_pe::id ()
{
    return authenticode_engine_id_t::authenticode_engine_id_pe;
}
