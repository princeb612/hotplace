...
/**
 * @file   quic_frame_new_connection_id.cpp
...
return_t quic_frame_new_connection_id::do_read_body(tls_direction_t dir, const byte_t* stream, size_t size, size_t& pos) {
    ...
    __try2 {
        ...
        secrets.assign(tls_secret_t::server_cid, std::move(connection_id));
        secrets.assign(tls_secret_t::stateless_reset_token, std::move(stateless_reset_token));
    }
    __finally2 {}
    return ret;
}
...
return_t quic_frame_new_connection_id::do_write_body(tls_direction_t dir, binary_t& bin) {
    ...
    __try2 {
        ...
        secrets.assign(tls_secret_t::server_cid, std::move(cid));
        secrets.assign(tls_secret_t::stateless_reset_token, std::move(token));
    }
    __finally2 {}
    return ret;
}
