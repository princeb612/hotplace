...
/**
 * @file   quic_frame_http3_stream.cpp
            ...
            switch (unitype) {
                case h3_control_stream: {
                    auto lambda_decoder = [&](const binary_t& bin, size_t& pos) -> return_t {
                        ...
                        return encoder.decode(&dyntable, bin.data(), bin.size(), pos, kv, qpack_quic_stream_decoder);
                    };
                    auto lambda_encoder = [&](const binary_t& bin, size_t& pos) -> return_t {
                        qpack_encoder encoder;
                        std::list<http_compression_decode_t> kv;
                        auto& dyntable = session->get_quic_session().get_dynamic_table();
                        return encoder.decode(&dyntable, bin.data(), bin.size(), pos, kv, qpack_quic_stream_encoder);
                    };
                    auto lambda_control = [&](const binary_t& bin, size_t& pos) -> return_t {
                        http3_frames frames;
                        return frames.read(session, bin.data(), bin.size(), pos);
                    };

                    if (quic_stream_unidirectional == (stream_id & quic_stream_unidirectional)) {
                        switch (unitype) {
                            case h3_qpack_decoder_stream: {
                                rc = streams.consume(stream_id, lambda_decoder);
                            } break;
                            case h3_qpack_encoder_stream: {
                                rc = streams.consume(stream_id, lambda_encoder);
                            } break;
                            case h3_push_stream: {
                                rc = errorcode_t::not_implemented;
                            } break;
                            case h3_control_stream: {
                                rc = streams.consume(stream_id, lambda_control);
                            } break;
                        }
                    } else {
                        rc = streams.consume(stream_id, lambda_control);
