...
dtls_server_socket::dtls_server_socket(transport_layer_security* tls) : _tls(tls) {
    if (nullptr == tls) {
        throw errorcode_t::insufficient;
    }
    tls->addref();
...
int dtls_server_socket::addref() { return _shared.addref(); }

int dtls_server_socket::release() { return _shared.delref(); }
...
return_t dtls_server_socket::close(socket_t sock, tls_context_t* tls_handle) {
    ...
    __try2 {
        ...
        udp_server_socket::close(sock, tls_handle);
    }
    __finally2 {
        // do nothing
    }
