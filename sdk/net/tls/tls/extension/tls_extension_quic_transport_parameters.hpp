...
/**
 * @file   tls_extension_quic_transport_parameters.hpp
...
class tls_extension_quic_transport_parameters : public tls_extension {
   public:
    ...
    static return_t read_quic_params(const byte_t* stream, size_t size, size_t& pos, std::list<std::pair<uint64, variant>>& params);
    static return_t write_quic_param(uint64 id, const variant& value, binary_t& params);
   ...
   private:
    ...
    std::list<std::pair<uint64, variant>> _params;
};
