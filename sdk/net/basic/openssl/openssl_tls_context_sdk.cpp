...
/**
 * @file   openssl_tls_context_sdk.cpp
...
void set_info_callback_routine(const SSL* ssl, int where, int ret) {
    __try2 {
        uint32 option = get_trace_option();
        if (trace_option_t::trace_debug & ~option) {
            __leave2;
...
int set_cookie_generate_callback_routine(SSL* ssl, unsigned char* cookie, unsigned int* cookie_len) {
    binary_t bin;
...
int set_cookie_verify_callback_routine(SSL* ssl, const unsigned char* cookie, unsigned int cookie_len) {
    int ret = 0;
...
void keylog_callback_routine(const SSL* ssl, const char* line) {
    if (line) {
...
int set_default_passwd_callback_routine(char* buf, int num, int rwflag, void* userdata) {
