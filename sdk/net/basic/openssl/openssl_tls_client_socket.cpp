...
/**
 * @file   openssl_tls_client_socket.cpp
...
openssl_tls_client_socket::openssl_tls_client_socket(openssl_tls* tls) : client_socket(), _tls(tls), _handle(nullptr) {
    if (nullptr == tls) {
...
openssl_tls_client_socket::~openssl_tls_client_socket() { _tls->release(); }

return_t openssl_tls_client_socket::connect(const char* address, uint16 port, uint32 timeout) {
...
return_t openssl_tls_client_socket::close() {
    return_t ret = errorcode_t::success;
    __try2 {
        _tls->close(_handle);
        _handle = nullptr;
...
return_t openssl_tls_client_socket::read(char* ptr_data, size_t size_data, size_t* cbread) {
    return_t ret = errorcode_t::success;
    __try2 {
        /*
...
return_t openssl_tls_client_socket::more(char* ptr_data, size_t size_data, size_t* cbread) {
    return_t ret = errorcode_t::success;
    __try2 {
...
return_t openssl_tls_client_socket::send(const char* ptr_data, size_t size_data, size_t* cbsent) {
    return_t ret = errorcode_t::success;
    __try2 {
