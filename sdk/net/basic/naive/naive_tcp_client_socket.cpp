...
/**
 * @file   naive_tcp_client_socket.cpp
...
return_t naive_tcp_client_socket::close() {
    return_t ret = errorcode_t::success;
    __try2 {
        if (INVALID_SOCKET == _fd) {
            __leave2;
...
return_t naive_tcp_client_socket::read(char* ptr_data, size_t size_data, size_t* cbread) {
    return_t ret = errorcode_t::success;
...
return_t naive_tcp_client_socket::more(char* ptr_data, size_t size_data, size_t* cbread) {
    return_t ret = errorcode_t::success;
    ret = read(ptr_data, size_data, cbread);
    return ret;
}

return_t naive_tcp_client_socket::send(const char* ptr_data, size_t size_data, size_t* cbsent) {
    return_t ret = errorcode_t::success;
    __try2 {
        if (INVALID_SOCKET == _fd) {
            ret = errorcode_t::not_open;
