...
/**
 * @file   naive_udp_client_socket.cpp
...
return_t naive_udp_client_socket::close() {
    return_t ret = errorcode_t::success;
    __try2 {
        if (INVALID_SOCKET != _fd) {
            ret = errorcode_t::already_assigned;
...
return_t naive_udp_client_socket::recvfrom(char* ptr_data, size_t size_data, size_t* cbread, struct sockaddr* addr, socklen_t* addrlen) {
    return_t ret = errorcode_t::success;
    __try2 {
        if (INVALID_SOCKET == _fd) {
...
return_t naive_udp_client_socket::sendto(const char* ptr_data, size_t size_data, size_t* cbsent, const struct sockaddr* addr, socklen_t addrlen) {
    return_t ret = errorcode_t::success;
    __try2 {
        if (INVALID_SOCKET == _fd) {
            ret = errorcode_t::not_open;
