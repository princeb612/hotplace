/* vim: set tabstop=4 shiftwidth=4 softtabstop=4 expandtab smarttab : */
/**
 * @file   http2_frame_data.hpp
 * @author Soo Han, Kim (princeb612.kr@gmail.com)
 * @desc
 *
 * Revision History
 * Date         Name                Description
 */

#ifndef __HOTPLACE_SDK_NET_HTTP_HTTP2_HTTP2FRAMEDATA__
#define __HOTPLACE_SDK_NET_HTTP_HTTP2_HTTP2FRAMEDATA__

#include <hotplace/sdk/net/http/http2/http2_frame.hpp>

namespace hotplace {
namespace net {

/**
 * @brief   data frame
 * @see
 *          RFC 7540 6.1. DATA
 *          RFC 7540 Figure 6: DATA Frame Payload
 */
class http2_frame_data : public http2_frame {
   public:
    http2_frame_data();
    http2_frame_data(const http2_frame_data& other);
    virtual ~http2_frame_data();

    virtual void dump(stream_t* s);

    void set_data(const binary_t& data);
    void set_data(const char* data, size_t size);
    const binary_t& get_data();

   protected:
    virtual return_t do_read_body(const byte_t* stream, size_t size, size_t& pos);
    virtual return_t do_write_body(binary_t& body);

   private:
    uint8 _padlen;
    binary_t _data;
};

}  // namespace net
}  // namespace hotplace

#endif
