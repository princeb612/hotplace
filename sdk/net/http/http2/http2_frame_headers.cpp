...
void http2_frame_headers::dump(stream_t* s) {
    if (s) {
        ...
        auto lambda = [&s](const std::string& name, const std::string& value) -> void { s->printf(" > %s: %s\n", name.c_str(), value.c_str()); };
...
const binary_t& http2_frame_headers::get_fragment() const { return _fragment; }
