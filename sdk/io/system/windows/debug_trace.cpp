...
return_t debug_trace::trace (debug_trace_context_t* handle, CONTEXT* rtlcontext, stream_t* stream)
{
    ...
    __try2
    {
        ...
        for (nFrameNum = 0; ; ++nFrameNum) {
            bRet = context->mssdk.lpfnStackWalk (image_type, process_handle, thread_handle, &frame, rtlcontext, nullptr, context->mssdk.lpfnSymFunctionTableAccess, context->mssdk.lpfnSymGetModuleBase, nullptr);
            if (FALSE == bRet) {
                break;
            }
