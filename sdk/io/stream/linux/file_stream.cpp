...
bool file_stream::is_open() const { return (-1 != _file_handle) ? true : false; }
...
void file_stream::seek(int64 file_pos, uint32 method) {
    if (true == is_open()) {
        lseek(_file_handle, file_pos, method);
    }
}
...
return_t file_stream::write(const void* data, size_t size) {
    ...
    int len = 0;
    while (size) {
        len = ::write(_file_handle, (byte_t*)data + pos, size);
        if (-1 == len) {
            ret = errno;
            break;
        }
...
return_t file_stream::read(void* data, size_t size, size_t* size_read) {
    ...
    __try2 {
        ...
        int ret_readfile = ::read(_file_handle, data, size);
        if (-1 == ret_readfile) {
            ret = errno;
            __leave2;
        }
