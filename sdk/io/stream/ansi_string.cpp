...
ansi_string::ansi_string (size_t allocsize, uint32 flags)
{
    _bio.open (&_handle, allocsize, sizeof (char), flags | bufferio_context_flag_t::memzero_free);
}
...
return_t ansi_string::write (void* data, size_t size)
{
...
size_t ansi_string::find (char* data)
{
    return _bio.find_first_of (_handle, data);
}

return_t ansi_string::replace (const char* from, const char* to, size_t begin, int flag)
{
    return _bio.replace (_handle, from, to, begin, flag);
}
...
return_t ansi_string::cut (size_t begin, size_t len)
{
    return _bio.cut (_handle, begin, len);
}
...
size_t ansi_string::find_first_of (const char* find, size_t offset)
{
    return _bio.find_first_of (_handle, find, offset);
}

size_t ansi_string::find_not_first_of (const char* find, size_t offset)
{
    return _bio.find_not_first_of (_handle, find, offset);
}

size_t ansi_string::find_last_of (const char* find)
{
    return _bio.find_last_of (_handle, find);
}

size_t ansi_string::find_not_last_of (const char* find)
{
    return _bio.find_not_last_of (_handle, find);
}
...
int ansi_string::compare (ansi_string& lhs, ansi_string& rhs)
{
...
{
    return 0 < strcmp ((*this).c_str (), buf.c_str ());
}
