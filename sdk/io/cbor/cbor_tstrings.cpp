...
cbor_tstrings::cbor_tstrings() : cbor_object(cbor_type_t::tstrs, cbor_flag_t::cbor_indef) {}
...
return_t cbor_tstrings::join(cbor_object* object, cbor_object* extra) {
    ...
    __try2 {
        ...
        if (cbor_type_t::data == object->type()) {
            ...
            if (vartype_t::TYPE_STRING == inst->data().type()) {
...
void cbor_tstrings::represent(binary_t* b) {
    ...
    if (b) {
        enc.encode(*b, cbor_major_t::tstr, cbor_control_t::cbor_control_begin, this);
        ...
        enc.encode(*b, cbor_major_t::tstr, cbor_control_t::cbor_control_end, this);
    }
}
