...
odbc_query::odbc_query (HDBC dbc_handle, sql_query_mode_t sql_mode)
    : _dbc_handle (dbc_handle), _stmt_handle (SQL_NULL_HSTMT), _sql_mode (sql_mode)
{
...
return_t odbc_query::build_fieldinfo (void)
{
    ...
    __try2
    {
        //ret_sql = SQLColAttributes(_stmt_handle, 0, SQL_COLUMN_COUNT, ucOwner, RTL_NUMBER_OF(ucOwner), &Scale, &ColLen ); /* MySQL ODBC 5.3 ANSI Driver return -1 */
        ...
        while (nCol <= NumResCols) {
            ret_sql = SQLDescribeCol (_stmt_handle, nCol, reinterpret_cast<SQLTCHAR*>(ColName), RTL_NUMBER_OF (ColName), &ColNameLength, &ColType, &ColSize, &Scale, &Nullable);
...
void odbc_query::clear_fieldinfo (void)
{
    _field_information.clear ();
}
...
return_t odbc_query::bind_statement_parameter (UINT nIndex, DWORD CType, DWORD SQLType, void* pData, size_t sizeData)
{
    ...
    __try2
    {
        ...
        ret_sql = SQLBindParameter (_stmt_handle, (SQLUSMALLINT) nIndex, SQL_PARAM_INPUT, (SQLSMALLINT) CType, (SQLSMALLINT) SQLType, 0, 0, pData, (SQLINTEGER) sizeData, nullptr);
        if (SQL_SUCCEEDED (ret_sql)) {
            // do nothing
        } else {
            ret = errorcode_t::internal_error;
...
odbc_record* odbc_query::get_fieldinfo ()
{
    return &_field_information;
}
...
int odbc_query::addref ()
{
    return _shared.addref ();
}

int odbc_query::release ()
{
        ...
        while (nCol <= columns) {
            ...
            __try2
            {
                ...
                if (dwDataSize == 0) {  /* 미리 준비한 버퍼를 사용해 데이터를 가져온 경우 */
                    __try_new_catch (odbc_field_ptr, new odbc_field (nCol, nColumnType, nColumnType, nDataLen > 0 ? (int) nDataLen : 1, (unsigned char*) DataBuf, pFieldRef), ret, __leave2);
                } else {                /* 메모리 스트림을 사용해 큰 데이터를 가져온 경우 */
                    __try_new_catch (odbc_field_ptr, new odbc_field (nCol, nColumnType, nColumnType, dwDataSize > 0 ? (int) dwDataSize : 1, (unsigned char*) pData, pFieldRef), ret, __leave2);
                }
