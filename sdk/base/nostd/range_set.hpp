...
class t_range_set {
   public:
    ...
    bool is_modified() const { return _status & modified; }
    void set_status(uint8 status) { _status = status; }
    uint8 get_status() const { return _status; }
   ...
   protected:
    void merge_internal() {
        // skip sort/merge if no changes
        if (is_modified() && (false == _arr.empty())) {
            ...
            _arr.resize(index + 1);
            set_status(get_status() & ~modified);  // remove modified flag
        }
    }
