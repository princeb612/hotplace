...
/*
 * @brief   int8~int128, uint8~uint128
 * @example
 *          binary_append(bin, i8);
 *          binary_append(bin, ui8);
 *          binary_append(bin, i16);
 *          binary_append(bin, ui16);
 *          binary_append(bin, i16, hton16);
 *          binary_append(bin, ui16, hton16);
 *          binary_append(bin, i32);
 *          binary_append(bin, ui32);
 *          binary_append(bin, i32, hton32);
 *          binary_append(bin, ui32, hton32);
 *          binary_append(bin, i64);
 *          binary_append(bin, ui64);
 *          binary_append(bin, i64, hton64);
 *          binary_append(bin, ui64, hton64);
 *          #if defined __SIZEOF_INT128__
 *          binary_append(bin, i128);
 *          binary_append(bin, ui128);
 *          binary_append(bin, i128, hton128);
 *          binary_append(bin, ui128, hton128);
 *          #endif
 */

template <typename T, typename transformer_t, typename = typename std::enable_if<t_is_integral<T>::value>::type>
return_t binary_append(binary_t& target, T value, transformer_t func) {
    using make_unsigned_t = typename t_make_unsigned<T>::type;
...
template <typename T, typename = typename std::enable_if<t_is_integral<T>::value>::type>
...
return_t binary_append(binary_t& target, T value, uint32 (*func)(uint32) = nullptr) {
    auto final_value = binary32_from_fp32(value);
    return binary_append(target, final_value, func);
}

template <typename T, typename = typename std::enable_if<std::is_same<T, double>::value>::type>
return_t binary_append(binary_t& target, T value, uint64 (*func)(uint64) = nullptr) {
    auto final_value = binary64_from_fp64(value);
    return binary_append(target, final_value, func);
}
 ...
 *          // 00000000 : 56 78 -- -- -- -- -- -- -- -- -- -- -- -- -- -- | Vx
 *          binary_t bin;
 *          binary_append_n<uint32>(bin, sizeof(uint16), ui32, hton32);
 *
 *          // wide
 *          // 00000000 : 00 00 00 00 12 34 56 78 -- -- -- -- -- -- -- -- | .....4Vx
 *          binary_append_n<uint32>(bin, sizeof(uint64), ui32, hton32);
 *          bin.clear();
 *
 *          uint128 ui128 = t_htoi<uint128>("0123456789abcdef");
...
return_t binary_load(binary_t& target, size_t bnlen, T value, transformer_t func) {
    ...
    if (0 != bnlen) {
        ...
        memcpy(target.data() + (bnlen - tsize), reinterpret_cast<const byte_t*>(&final_value) + toffset, tsize);
    }
    return errorcode_t::success;
}
