...
static inline bool ends_with (std::string const & source, std::string const & suffix)
{
    return source.rfind (suffix) == (source.size () - suffix.size ());
}

static inline bool ends_with (std::wstring const & source, std::wstring const & suffix)
{
    return source.rfind (suffix) == (source.size () - suffix.size ());
}
