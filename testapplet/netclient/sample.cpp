...
int main(int argc, char** argv) {
    ...
    (*_cmdline)
        ...
        << t_cmdarg_t<OPTION>("-b", "bufsize (1500)", [](OPTION& o, const char* param) -> void { o.bufsize = atoi(param); }).optional().preced()
        << t_cmdarg_t<OPTION>("-a", "address (127.0.0.1)", [](OPTION& o, const char* param) -> void { o.address = param; }).optional().preced()
        ...
        << t_cmdarg_t<OPTION>("-P", "protocol tcp|udp|tls|tls13|tls12|dtls|quic (1 tcp, 2 udp, 3 tls, 4 dtls, 5 quic)",
                              [](OPTION& o, const char* param) -> void { o.prot = toprot(o, param); })
               .preced()
        << t_cmdarg_t<OPTION>("-c", "count (1)", [](OPTION& o, const char* param) -> void { o.count = atoi(param); }).optional().preced()
        << t_cmdarg_t<OPTION>("-wto", "wait time out (1000 milli-seconds)", [](OPTION& o, const char* param) -> void { o.wto = atoi(param); }).optional().preced()
        << t_cmdarg_t<OPTION>("-k", "keylog", [](OPTION& o, const char* param) -> void { o.flags |= option_flag_keylog; }).optional()
        << t_cmdarg_t<OPTION>("-T", "use trial", [](OPTION& o, const char* param) -> void { o.flags |= option_flag_debug_tls_inside; }).optional()
        << t_cmdarg_t<OPTION>("-h", "HTTP/1.1",
               ...
               .optional()
        << t_cmdarg_t<OPTION>("-m", "message", [](OPTION& o, const char* param) -> void { o.message = param; }).optional().preced();
