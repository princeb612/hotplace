...
void test_keyexchange_mlkem(tls_group_t group) {
    ...
    size_t sharesize = hint->first.keysize + hint->second.keysize;
    size_t capsulesize = 0;
    if (kty_mlkem == hint->first.kty) {
        capsulesize = hint->first.capsulesize + hint->second.keysize;
    } else {
        capsulesize = hint->first.keysize + hint->second.capsulesize;
    }
