...
void do_test_aes128cbc_mac_routine(const binary_t& key, const binary_t& message, const binary_t& expect) {
    ...
    ret = hash.open(&handle, crypt_algorithm_t::aes128, crypt_mode_t::cbc, key.data(), key.size());
    if (errorcode_t::success == ret) {
        // Figure 2.3.  Algorithm AES-CMAC
        hash.init(handle);
        hash.update(handle, message.data(), message.size());
...
void testcase_rfc4493() { test_rfc4493(); }
