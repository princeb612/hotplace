...
void test_range_set2() {
...

    {
        ...
        _test_case.assert(res[0] == t_range_set<size_t>::interval(0, 31), __FUNCTION__, "res[0]");
    }

    _test_case.begin("merge (subtract)");

    {
        t_range_set<uint8> rs;
        ...
        _test_case.assert(res[0] == t_range_set<uint8>::interval(1, 2), __FUNCTION__, "res[0]");
        _test_case.assert(res[1] == t_range_set<uint8>::interval(8, 10), __FUNCTION__, "res[1]");
    }

    {
        ...
        _test_case.assert(1 == res.size(), __FUNCTION__, "res");
        _test_case.assert(res[0] == t_range_set<int>::interval(8, 10), __FUNCTION__, "res[0]");
    }

    {
        ...
        _test_case.assert(2 == res.size(), __FUNCTION__, "res");
        _test_case.assert(res[0] == t_range_set<int>::interval(-10, -4), __FUNCTION__, "res[0]");
        _test_case.assert(res[1] == t_range_set<int>::interval(9, 10), __FUNCTION__, "res[1]");
    }
}

void test_ack() {
    _test_case.begin("point set");

    auto lambda = [](const char* func, const char* text, t_point_set<uint32>& p, ack_t& e) -> void {
        ack_t ack;
        ack << p;

        t_point_set<uint32> temp;
        ack >> temp;
...
void test_subtraction() {
    _test_case.begin("set subtraction");
    // [retransmission] check PKN not acknowledged

    {
        t_range_set<uint32> rs;
        rs.add(7, 12).add(15, 16).add(17, 18).add(19, 25);
        rs.subtract(7, 11).subtract(16, 16).subtract(17, 19).subtract(23, 25).for_each([](uint32 s, uint32 e) -> void {
            if (s == e) {
                _logger->writeln("> %u", s);
            } else {
...

    {
        t_range_set<uint32> rs;
        rs.add(7, 12).add(15, 16).add(17, 18).add(19, 25);
        t_range_set<uint32> ovl2;
        ovl2.add(7, 11).add(16, 16).add(17, 19).add(23, 25);
        rs.subtract(ovl2).for_each([](uint32 s, uint32 e) -> void {
            if (s == e) {
                _logger->writeln("> %u", s);
            } else {
                _logger->writeln("> %u-%u", s, e);
