...
/**
 * @file   testcase_understand_tls12.cpp
...
void test_tls12_xargs_org() {
    ...
    // https://tls12.xargs.org/#server-certificate
    {
        const char* record =
            ...
            "08 46 36 A0 -- -- -- -- -- -- -- -- -- -- -- --";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("certificate", &session, from_server, bin_record);
    ...
    // https://tls12.xargs.org/#server-hello-done
    {
        ...
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("server_hello_done", &session, from_server, bin_record);
    ...
    // https://tls12.xargs.org/#client-key-exchange
    {
        ...
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("client_key_exchange", &session, from_client, bin_record);
    ...
    // https://tls12.xargs.org/#client-handshake-finished
    {
        const char* record =
            ...
            "90 DE DF 72 0F -- -- -- -- -- -- -- -- -- -- --";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("client finished", &session, from_client, bin_record);
    }
    ...
    }  // https://tls12.xargs.org/#server-change-cipher-spec
    {
        const char* record = "14 03 03 00 01 01";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("server change-cipher-spec", &session, from_server, bin_record);
    }
    // https://tls12.xargs.org/#server-handshake-finished
    {
        const char* record =
            ...
            "4B D4 94 CF 7D -- -- -- -- -- -- -- -- -- -- --";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("server finished", &session, from_server, bin_record);
    }
    // https://tls12.xargs.org/#client-application-data
    {
        const char* record =
            ...
            "47 EC 79 0C 99 -- -- -- -- -- -- -- -- -- -- --";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("client application data", &session, from_client, bin_record);
    }
    // https://tls12.xargs.org/#server-application-data
    {
        ...
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("server application data", &session, from_server, bin_record);
    }
    // https://tls12.xargs.org/#client-close-notify
    {
        const char* record =
            ...
            "FD 12 87 A6 2D -- -- -- -- -- -- -- -- -- -- --";
        binary_t bin_record = std::move(base16_decode_rfc(record));
        dump_record("client close notify", &session, from_client, bin_record);
    }
}

void testcase_understand_tls12() { test_tls12_xargs_org(); }
